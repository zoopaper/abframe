                ...
                <form action="news/list?isM1=${pdm.isM1}&isM2=${pdm.isM2}" method="post" name="userForm"
                    ...
                    <table border='0'>
						...
						<span class="input-icon">
                            ...
                            <td style="vertical-align:top;">
                                <a class="btn btn-mini btn-light" onclick="createHtml();" title="生成html">
                                    <i id="nav-search-icon" class="icon-cog"></i>
                                </a>
                            </td>
                        </tr>
                    </table>
...
<script type="text/javascript">
    ...
    function edit(id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>news/toEdit?id=' + id;
        diag.Width = 800;
        diag.Height = 650;
        diag.CancelEvent = function () {
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
    ...
    function del(id) {
        bootbox.confirm("确定要删除该记录?", function (result) {
            if (result) {
                var url = "<%=basePath%>news/delete?id=" + id + "&tm=" + new Date().getTime();
    ...
    function createHtml() {
        bootbox.confirm("确定要生成html?", function (result) {
            if (result) {
                var url = "<%=basePath%>news/createHtml?tm=" + new Date().getTime();
                $.get(url, function (data) {
                    if (data == "success") {
                        alert("OK");
                    }
